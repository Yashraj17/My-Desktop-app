...
const Drawer = ({ isOpen, onClose, title, children }) => {
  ...
  return (
    ...
    >
      ...
      {/* Drawer Panel */}
      <div
        className={`absolute top-0 right-0 h-full w-full max-w-md bg-white dark:bg-gray-800 shadow-xl transform transition-transform duration-300 ease-in-out ${
...
export default function Menus() {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-gray-600 whitespace-nowrap">
          AED {Number(row.getValue("price")).toFixed(2)}
        </div>
      ),
      width: "100px",
    },
  ...
  const loadData = async () => {
    try {
      ...
      if (selectedCategory) {
        const activeMenu = menusWithImagePaths.find(
          (m) => m.id === selectedCategory.id
        );
        if (activeMenu) {
  ...
  useEffect(() => {
    if (selectedCategory) {
      ...
      if (activeMenu) {
        ...
        const filtered = activeMenu.items.filter(
          (item) =>
      ...
      <div className="grid p-5 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {menus.map((menu) => (
          <div
            ...
            className={`p-3 rounded-lg flex items-center space-x-4 cursor-pointer transition-all duration-200 border ${
              selectedCategory && selectedCategory.id === menu.id
