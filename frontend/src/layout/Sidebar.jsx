...
export default function Sidebar({ activeMenu = "/", onMenuClick, routes = [], currentPath = "/", collapsed = false }) {
  ...
  useEffect(() => {
    ...
    if (shouldOpenDropdown) {
      ...
      const currentRoute = routes.find((route) => location.pathname === route.url)
  ...
  return (
    <div
      className={`h-full flex flex-col border-r-2 transition-all duration-300 ease-in-out bg-gray-50 dark:bg-gray-800 border-gray-300 dark:border-gray-600 ${collapsed ? "w-[60px]" : "w-62"}`}
    >
      {/* Navigation Items */}
              ...
              {!collapsed && item.hasDropdown && item.children && (
                <div
                  className={`overflow-hidden transition-all duration-300 ease-in-out ${
                    openDropdowns.includes(item.label) ? "max-h-48 opacity-100" : "max-h-0 opacity-0"
                  }`}
                >
      ...
      <div className="flex-shrink-0">
        ...
        >
          ...
          <ExternalLink className={`h-4 w-4 transition-all duration-300 ${collapsed ? "ml-0" : "ml-2"}`} />
        </Button>
      </div>
    </div>
  )
